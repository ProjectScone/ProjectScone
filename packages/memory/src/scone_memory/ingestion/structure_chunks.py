...
def units(content: str, limit: int = MAX_SECTIONS, *, reader: UnitReader | None = None) -> tuple[Unit, ...]:
    ...
    for index in range(begin, len(lines)):
        ...
        if byte in heads:
            kind, depth = ("heading", 0) if reader is None else reader.heading(heads[byte], levels[byte])
    ...
    for position, (start, kind, label, depth) in enumerate(found):
        ...
        out.append(Unit(at[start], _point(at, end, content), kind, label, depth))
        if end < after:
