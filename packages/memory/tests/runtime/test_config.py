...
async def test_exact_word_forms_are_a_flag_that_reaches_every_engine():
    ...
    settings = Settings.from_env({"SCONE_LEXICAL_EXACT_FORMS": "1"})
    assert settings.lexical_exact_forms is True and Settings.from_env({}).lexical_exact_forms is False
    assert "lexical_exact_forms" in ENGINE_SETTINGS
    engine = await build_engine(settings)
    try:
        assert engine.lexical_exact_forms is True
    finally:
        await engine.close()
    assert (await build_in_process_engine(settings, HashEmbedder())).lexical_exact_forms is True
    assert (await build_in_process_engine(Settings.from_env({}), HashEmbedder())).lexical_exact_forms is False
