...
def test_two_connections_only_one_claim(approvals,tmp_path):
    ...
    def claim():
        ...
        try:
            ...
            try:
                return AgentApprovalStore(local).claim('alpha','one',record.request_id,activation_id='continue',call=call).revision
    ...
    def operation(cancel):
        ...
        try:
            ...
            if cancel:
                ...
                return 'cancelled'
            try:return AgentApprovalStore(local).claim('alpha','one',record.request_id,activation_id='continue',call=call).revision
            except WorkflowError as error:return error.code
        finally:local.close()
    with concurrent.futures.ThreadPoolExecutor(max_workers=2) as pool:
...
def test_activation_rolls_back_all_records(approvals):
    ...
    records=[store.request('alpha','one',call),store.request('alpha','one',call.model_copy(update={'operation_digest':'c'*64}))]
    for record in records:store.decide('alpha','one',record.request_id,decision='approve',actor='owner',expected_revision=1)
    original=store._put
    count=0
    def fail(db,record):
        nonlocal count
        count+=1
        if count==2:raise sqlite3.OperationalError('private SQL detail')
        original(db,record)
    store._put=fail
    try:
...
def test_claim_lost_commit_ack_is_not_reclaimable(approvals):
    ...
    def fail(*,write=False):
        with original(write=write) as db:yield db
        if write:raise WorkflowError('run_store_unavailable')
    runs._storage._access=fail
    try:
        with pytest.raises(WorkflowError,match='run_store_unavailable'):
            store.claim('alpha','one',record.request_id,activation_id='continue',call=call)
    finally:runs._storage._access=original
    assert store.get('alpha','one',record.request_id).revision==4
    with pytest.raises(WorkflowError,match='approval_already_claimed'):
        store.claim('alpha','one',record.request_id,activation_id='continue',call=call)
    assert store.activate('alpha','one','continue',decisions={record.request_id:2}).activation_id=='continue'
