...
from scone_memory.retrieval.reading_order import arranged, ends_first, validate_reading_order
...
def test_ends_first_puts_odd_ranks_at_the_front_and_even_ranks_at_the_back_and_keeps_every_item(length):
    ranked = list(range(1, length + 1))
    order = ends_first(ranked)
    assert sorted(order) == ranked and len(order) == length
    if length >= 4:
        assert order[0] == 1 and order[-1] == 2 and order[1] == 3 and order[-2] == 4
    assert arranged(ranked, "ranked") == ranked and arranged(ranked, "ends") == order
