...
counts, every recall's event says what it was, and nothing changes at
the default.
"""
...
async def test_the_weight_is_on_every_recall_event_and_is_bounded():
    ...
    assert event.payload["fusion_weights"] == {"vector": 0.25, "text": 1.0}, "unset, a hashed-token embedder's lane gets a quarter voice"
...
async def test_the_default_voice_follows_the_embedder_and_is_on_the_record():
    ...
    assert hashed.vector_weight == HASHED_VECTOR_WEIGHT == 0.25, "a hashed-token embedder is a weak echo of the text lane"
    await hashed.recall("s", "billing", limit=1)
    [event] = await hashed.events.query("s", kind="recall")
    assert event.payload["fusion_weights"] == {"vector": 0.25, "text": 1.0}
