...
def test_the_callers_filters_are_kept_over_the_questions():
    ...
    kept = apply_scope(scope, since="2026-01-01T00:00:00Z", until=None, kind="file", tags=["urgent", "x"], source_prefix="src/")
    assert (kept.since, kept.until, kept.kind, kept.tags, kept.source_prefix) == ("2026-01-01T00:00:00Z", None, "file", ("urgent", "x"), "src/")
...
async def test_the_route_and_the_command_search_with_what_the_question_says_and_say_so(tmp_path):
    ...
    try:
        await engine.remember("s", "The harbour closes to sailing boats every November.", kind="note")
