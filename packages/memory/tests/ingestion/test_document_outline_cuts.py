...
from scone_memory.ingestion.document_outline import DocumentOutline, Heading, outline
...
async def test_the_headings_kept_for_reuse_are_bounded(monkeypatch):
    ...
    monkeypatch.setattr(batch, "OUTLINES_READ_MAX", 2)
    asked = []

    async def read(episode):
        asked.append(episode.content_hash)
        return DocumentOutline(headings=(Heading(0, 1, episode.content_hash),))
    memory, _ = await engine_with()
    try:
        runtime = memory._ingestion_runtime()
        runtime = batch.IngestionRuntime(**{**runtime.__dict__, "document_outline": read, "outlines_read": {}})
        ...
        for episode in episodes:
            ...
            assert (await batch.headings_of(runtime, episode))[0].title == episode.content_hash
        assert asked == ["a", "b", "c"] and len(runtime.outlines_read) == 2
